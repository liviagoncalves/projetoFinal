...
public class GerenciarMenuPerfil extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (request.getParameter("acao").equals("gerenciar")) {
                if(GerenciarLogin.verificarPermissao(request, response)){
                    Perfil p = pDAO.getPerfilPorIdperfil(Integer.parseInt(idperfil));
                    if (p.getIdperfil() > 0) {
            ...
            if (request.getParameter("acao").equals("desvincular")) {
                if(GerenciarLogin.verificarPermissao(request, response)){
                    String idmenu = (request.getParameter("idmenu"));
                    if (idmenu.equals("")) {
                        mensagem = "Campos idmenu deverão ser selecionados";
                    } else {
                        if (pDAO.desvincular(Integer.parseInt(idmenu), Integer.parseInt(idperfil))) {
                            mensagem = "Desvinculado com sucessor!!!";
                        } else {
